...
interface SubjectProgress {
    ...
    total_attempts: number;
    total_correct: number;
    proficiency_score: number; // 0.0 to 1.0 (NEW)
}
...
interface DashboardData {
    ...
    achievements: { badges: number };
    last_weak_topics: string[];
    subject_progress: SubjectProgress[]; // NEW
}
...
// Paste this section into your App.tsx file.
// It uses styled divs to represent charts, as installing a charting library is not feasible in a single file environment.
...
// --- QUIZ SUBJECT SELECTION SCREEN (UPDATED) ---
        ...
        try {
            const body = isRegisterMode
                ? {
                    username,
                    password,
                    // Only send pokemon_name if the theme is pokemon_kalos (default partner for OP is handled by backend)
                    pokemon_name: themeName === 'pokemon_kalos' ? pokemonName : 'Turtwig', 
                    theme: themeName
                }
                : {
